class SystemDebug
  ...
  if File.exist?('/opt/engines/etc/debug/debug_flags.rb')
    ...
    @@debug_flags = 0
   @@debug_flags =   @@orphans |@@container_events # |@@builder # |@@containers #|@@services  | @@container_events #|@@containers |@@engine_tasks # |@@builder |  @@container_events #  |  # @@first_run  |@@builder # @@actions# @@docker# @@builder  | @@docker | @@services | @@registry |@@containers
