module CheckBuildParams
  ...
  def check_host(params)
    return bad_param('Missing: Hostname', params) unless params.key?(:engine_name)
    return bad_param('Invalid: Hostname', params) unless acceptable_chars(params[:engine_name])
    return true
  end
