class ServiceBuilder < ErrorsApi
  def required_services_are_running?
    ...
    #  SystemDebug.debug(SystemDebug.builder, :checking_service, service_def[:service_container].to_s)
      ...
      raise EngineBuilderException.new(error_hash('Required Service not running ' + service_def[:service_container].to_s)) unless core.is_service_running?(service_def[:service_container])
    end
    true
  end
end
