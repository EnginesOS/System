...
#       File.delete(stateDir)
      ...
      cmd = "docker run -i --name volbuilder --memory=20m -e fw_user=www-data --cidfile /opt/engines/run/volbuilder.cid  -v /opt/engines/run/containers/" + container.containerName + "/:/client/state:rw  -v /var/log/engines/containers/" + container.containerName + ":/client/log:rw    -t engines/volbuilder /home/remove_container.sh state logs"  
      retval =  SystemUtils.run_system(cmd)
      if retval == true
        Dir.delete(container_state_dir(container))
      else
        SystemUtils.log_error_mesg("Failed to Delete state and logs:" + retval ,container)
        return false  
      end
