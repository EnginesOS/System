  ...
  def start_dependancies(container)
    ...
    container.dependant_on.each do |service_name|
     ...
     # retries = 0
      ...
      # use event queue
     # service =  @engines_core.loadManagedService(service_name)
      raise EnginesException.new(error_hash('Time out in waiting for Service Dependancy ' + service_name + ' to start ', service_name)) unless service.wait_for_startup(service, 120)
