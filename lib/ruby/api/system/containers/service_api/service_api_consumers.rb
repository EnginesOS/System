module ServiceApiConsumers
  ...
  def add_consumer_to_service(c, service_hash)
    ...
    STDERR.puts( ' service hash  add ' + cmd.to_s)  
    result =  engines_core.exec_in_container({:container => c, :command_line => cmd, :log_error => true , :timeout => @@consumer_timeout}) 
    ...
    return  log_error_mesg('Failed add_consumer_to_service ' + c.to_s + ':' + service_hash[:variables].to_s + ':' + result.to_s,result)
  end
...
#    cmd = 'docker_exec -u ' + c.cont_userid + ' ' + c.container_name + ' /home/rm_service.sh \'' + SystemUtils.hash_variables_as_json_str(service_hash) + '\''
    ...
    STDERR.puts( ' service hash rm ' + cmd.to_s)     
    result =  engines_core.exec_in_container({:container => c, :command_line => cmd, :log_error => true , :timeout => @@consumer_timeout} )
  return result if result.is_a?(EnginesError)
    return true  if result[:result] == 0
