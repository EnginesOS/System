...
module ServiceHashChecks
  ...
  def check_new_service_hash(service_hash)
    ...
    raise EnginesException.new({error_mesg: 'illegals chars in :service_handle [a-Z0-9_] only ',error_type: :error , params: service_hash}) unless service_hash[:service_handle].match(/^[a-zA-Z_]+$/)
  end
end