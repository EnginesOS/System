...
module ServiceHashChecks
  ...
  def check_new_service_hash(service_hash)
    ...
    raise EnginesException.new({error_mesg: 'illegals chars in :service_handle [a-zA-Z0-9_] only ' + service_hash[:service_handle].to_s ,error_type: :error, params: service_hash}) unless service_hash[:service_handle].match(/^[a-zA-Z_]+$/)
  end
end