class SystemApi
  ...
  def destroy_container container
    clear_error
      container.container_id=(-1)
      return File.delete(container_cid_file(container)) if File.exist?(container_cid_file(container)) == true
  ...
  def delete_container_configs(container)
    ...
    File.delete(cidfile) if File.exists?(cidfile)
...
#  end
  ...
  def get_build_report(engine_name)
    ...
    return File.read(stateDir  + '/buildreport.txt') if File.exists?(stateDir + '/buildreport.txt')
...
#      saved_domainName =  container.domain_name
  ...
  def list_managed_engines
    ...
    begin
      Dir.entries(SystemConfig.RunDir + '/containers/').each do |contdir|
        yfn = SystemConfig.RunDir + '/containers/' + contdir + '/running.yaml'
        ret_val.push(contdir)  if File.exists?(yfn) == true
      end
    rescue StandardError => e
      SystemUtils.log_exception(e)
      return ret_val
