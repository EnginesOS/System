  ...
  def backup_lastbuild buildname      
    ...
    def buildname
     ...
     def build_deploy
       ...
       cmd= "cd " + SysConfig.DeploymentDir + "/" +  @buildname + "; docker run --memory=32m --name deploy " + volumes + " -t " +   @hostName + "/setup /bin/bash /home/_init.sh " # su -s /bin/bash www-data /home/configcontainer.sh"
         puts(cmd) 
       res= %x<#{cmd}>
       puts res
                     if $? == false
