...
module Container
  class Store
    require_relative 'container_locking'
    protected
    def container_locking
        @container_locking ||= ContainerLocking.new
      end
      
    def unlock(lock_key)
      container_locking.unlock(lock_key)
    end
    def lock(lock_key)
      container_locking.lock(lock_key)
      end
    end
#    @@lock_timeout = 5
#
