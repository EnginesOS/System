module RunningContainerStatistics
  def stats
   ...
   #   SystemDebug.debug(SystemDebug.containers,'ps_container json result',container_name,ps_json)
      ...
      rss = 0
      cpu = 0
      if ps_json.nil? || ps_json.is_a?(FalseClass)
        ContainerStatistics.new(read_state, pcnt, started, stopped, rss, vss, cpu)
      else
  ...
  def get_container_memory_stats()
    container_dock.get_container_memory_stats(self)
  end

  def get_container_network_metrics()
    container_dock.get_container_network_metrics(self)
  end
