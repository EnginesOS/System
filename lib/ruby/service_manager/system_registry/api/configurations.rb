...
module Configurations
 # require_relative 'xcon_rset.rb'
  ...
  def add_service_configuration(config_hash)
    rest_post('services/configurations/add/' + config_hash[:service_name] + '/' + config_hash[:configurator_name] , {:api_vars => config_hash} )
    rescue StandardError => e
       STDERR.puts( 'Failed To services/configurations/add/  ' + config_hash.to_s)
        SystemUtils.log_exception(e)
  end
