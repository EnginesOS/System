...
module Configurations
 # require_relative 'xcon_rset.rb'
  ...
  def update_service_configuration(config_hash)
    rest_post('/v0/system_registry/services/configuration/update/' + config_hash[:service_name] + '/' + config_hash[:configurator_name] + '/' + config_hash[:publisher_namespace] + '/*', {:api_vars => config_hash} )
  end
  ...
  def add_service_configuration(config_hash)
    rest_post('/v0/system_registry/services/configurations/add/'+ config_hash[:service_name] + '/' + config_hash[:configurator_name]+ '/' + config_hash[:configurator_name] + '/' + config_hash[:publisher_namespace] + '/*', {:api_vars => config_hash} )
  end
