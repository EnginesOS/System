...
class CronService < SoftwareService
  ...
  #    service_hash[:service_type]='cron'
  ...
  def format_cron_line(cron_hash)
    ...
    cron_line +=" docker exec " +  cron_hash[:parent_engine] + " "
    n=5
  ...
  def remove_containers_cron_list(containerName)
    ...
    consumers.each do |cron_job|
      if cron_job != nil
        p cron_job
        p :looking_at
        p cron_job[1][:parent_engine]
        if cron_job[1][:parent_engine] ==  containerName
