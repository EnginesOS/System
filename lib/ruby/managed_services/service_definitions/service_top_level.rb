def set_top_level_service_params(service_hash, container_name)
  ...
  if service_def.key?(:service_handle_field) && !service_def[:service_handle_field].nil?
    ...
    raise EnginesException.new(error_hash('Missing Service Handle field in variables ' + service_hash[:variables].to_s , handle_field_sym)) unless service_hash[:variables].key?(handle_field_sym)
