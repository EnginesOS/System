...
if test $# -eq 1
  ...
  if ! test -d /opt/engines/run/apps/$1 &>> /tmp/create_container_dir
   ...
   if ! test -d /opt/engines/etc/ssh/keys/apps/$1 
   then
	mkdir /opt/engines/etc/ssh/keys/apps/$1  &>>  /tmp/create_container_dir
	chown -R engines.containers /opt/engines/etc/ssh/keys/apps/$1
	chmod g+w /opt/engines/etc/ssh/keys/apps/$1
  fi
