...
post '/v0/system/uadmin/*' do
  begin
    require_relative 'uadmin_verbs.rb'
    uadmin_response(uadmin_post(params[:splat][0], request.env['rack.input'], params[:api_vars]))
  rescue StandardError => e
    send_encoded_exception(request: request, exception: e)
  end
end
...
delete '/v0/system/uadmin/*' do
  begin
    require_relative 'uadmin_verbs.rb'
  uadmin_response(uadmin_del(params[:splat][0], {:api_vars, {:api_vars, params[:api_vars]}}))
  rescue StandardError => e
    send_encoded_exception(request: request, exception: e)
  end
end